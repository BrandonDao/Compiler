...
public class RegexLexer : ILexer
{
    ...
    {
        IOrderedEnumerable<TokenDefinition> orderedDefinitions = tokenDefinitions.OrderBy(d => d.Priority);
#if DEBUG
        if (!orderedDefinitions.Any())
        {
            throw new InvalidOperationException("No token definitions available!");
        }

        Type LeafNodeType = typeof(LeafNode);
        ...
        IEnumerable<Type> missing = nodeTypes.Except(tokenDefinitions.Select(def => def.NodeFactory(string.Empty, 0, 0, 0, 0).GetType()));

        if (missing.Any(t => !t.Name.Contains("Implicit")))
        {
            throw new InvalidOperationException($"Some token types are missing definitions: {string.Join(", ", missing)}");
        }
#endif
        ...
        for (int textIdx = 0; textIdx < text.Length;)
        {
            ...
            foreach (TokenDefinition? def in orderedDefinitions)
            {
